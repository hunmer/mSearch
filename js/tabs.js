var g_tabs = {
    ...
    group_clear: function(group) {
        ...
        div.find('[class="nav-item"]').remove()
    ...
    webview_bind: function(webview) {
        ...
        webview.addEventListener('dom-ready', function(e) {
            this.setAudioMuted(g_setting.getConfig('mute'))
            // 绑定webContentID
        ...
        webview.addEventListener('load-commit', function(e) {
            g_rule.matchWeb(nodejs.webContents.fromId(this.getWebContentsId()))
        });
    ...
    tab_add: function(group, opts = {}) {
        ...
        g_plugin.callEvent('beforeLoadURL', {
            ...
            let btn = $(this.get_html('group-item', { id: id, tid: tid, show: '', title: opts.title })).insertBefore(div.find('.nav-item.ms-auto')) // 设置按钮之前

            let con = $(this.get_html('group-content', { tid: tid, show: '', url: opts.url, show1: 'data-', preload: 'file://' + __dirname + '/js/webview.js', id: id })).appendTo(div.find('.tab-content'))
